...
//                 let rou = <F>::rou(1 << self.coeffs.len(), false);
...
//     pub fn combine(&mut self, mut argb: Subtree<F>, depth: usize) -> Subtree<F> {
...
//         let mut result = vec![<F>::zero(); self.coeffs.len() * 2];
//         let rou = <F>::rou(self.coeffs.len() * 2, false);
...
//     let mut result: Vec<Subtree<F>> = vec![rbe[0].clone()];
